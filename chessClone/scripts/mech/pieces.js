...
class Piece{
	...
	toggle(arg){
		...
		if( isGameOver == false && this.myTurn() == true ){
			...
			var rtnColor = ()=> this.parentTag.style.background
			var color = c => this.parentTag.style.background = c

			if(arg == "over"){ rtnColor() == "var(--verde)" ? color("var(--verdeEscuro)") : color("var(--cinzaClaro)") }

			if(arg == "out"){ rtnColor() == "var(--verdeEscuro)" ? color("var(--verde)") : color("var(--branco)") }
